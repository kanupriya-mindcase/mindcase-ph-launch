  ...
  "2026-08-21": {
    ...
    makers: [
      ...
      },
      {
        ...
        drafts: [
          "Saurabh here. One thing that got compressed into a single line in our post but deserves more: the custom agents. The 75+ ready ones cover the sources most people ask for, but the interesting requests are always the ones that are not on the list. Some internal directory, a regional marketplace, an industry site nobody outside that industry has heard of. For those we build the agent as an API for your specific use case, so you end up calling it exactly like linkedin/profiles rather than inheriting a scraper you now have to own. That distinction matters more than it sounds, because a scraper handed to you becomes your maintenance burden the first time the site changes, and an API does not. If you have a source like that, drop it in the comments with roughly what fields you need. I will tell you honestly whether it is feasible, and if it is not, why.",
          "Saurabh from Mindcase. The design constraint we kept coming back to was that adding a new source should cost you one line, not one sprint. That sounds obvious and it is surprisingly hard, because every site has its own pagination, its own auth quirks, its own idea of what an ID is, and the honest thing to do is absorb all of that rather than leak it into the interface. So the request shape is identical across every agent, the response is structured the same way, and switching sources does not ripple through your pipeline. Python and Node.js SDKs on top of that. The test I care about is whether someone can go from reading the docs to a working call without talking to us. If you try it today and that is not true, tell me exactly where you got stuck, because that is a bug in our work and not in your understanding."
        ]
      }
    ],
    comments: [
